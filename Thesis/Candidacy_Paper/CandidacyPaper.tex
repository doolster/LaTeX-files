...
Type theory is a foundation for mathematics which formalizes the notion of mathematical terms, say the number 2 or a function $f$, and the types which those terms belong to, say the natural numbers or functions $\N\to\N$. Dependent type theory is then a type theory in which the basic judgments are not only that a term has a type, but that a type itself may depend on a surrounding context of assumptions. In the fragment we reference throughout this paper, a typical judgment has the form
\[ \Gamma\vdash A\ \text{type}\qquad\text{or,}\qquad\Gamma\vdash a : A, \]
where $\Gamma$ is a context, i.e. a list of typed variables $\Gamma=(x_1:X_1,\dots,x_n:X_n)$, and $a$ or $A$ may implicitly depend on the variables $x_i$. The basic structural rules of type theory can be extended by the inclusion of type constructors which allow us access to various constructions within our theory such as a unit type, denoted 1, dependent pair and function types, called $\Sigma$ and $\Pi$ types respectively, or the type of identities on a given type. These constructions are asserted by adding rules to our theory such as the following for $\Id$-types.
...
These rules individually tell us how to form a type, how to introduce and eliminate terms of that type, and how those introduction and elimination rules should compute with one another. A presentation for the structural rules of Martin-L\"of Type Theory (MLTT), along with the rules for other common type constructors can be found in \cite[Appendix A]{KL21}.

A valuable aspect of dependent type theory is that it naturally admits a syntax-semantics relationship with category theory. On the syntactic side, contexts and substitutions naturally organize into a category we call the \textbf{syntactic category} of a type theory, whose objects are contexts and whose morphisms, from a context $\Gamma$ to a context $\Delta$, are lists of terms of the types in $\Delta$ derived in the context $\Gamma$. On the other side, we can get a type theory by considering the internal logic of a category with sufficient structure.

This syntax-semantics relationship has been well studied in a variety of contexts where it has been shown that the presence of certain logical structures within a theory corresponds to certain categorical constructions. An example of this relevant to the current paper is the relationship between locally Cartesian closed categories and MLTT with extensional identity types (meaning identity types with trivial higher structure). This result was formally made as an equivalence of the 2-categories of locally Cartesian closed categories and of a certain class of semantic models of extensional MLTT \cite{CD}. The purpose of this paper is to explain the conjectured equivalence one would get when allowing for the higher structure naturally present by iterating identity types, which should correspond to higher categorical structure on the semantic side.

As briefly referenced in the previously noted result, when studying type theories, we often do not study the syntax directly, but rather, we consider semantic models which closely represent the structures that we find present in the syntactic categories of type theories. Studying these semantic models has several advantages:
\begin{itemize}
    \item it produces a category of models with natural morphisms between them, which is essential for comparing theories and constructing a notion of homotopy;
    \item it makes available categorical tools such as (co)limits, adjoints, and homotopy theoretic constructions that can be hard to see directly in raw syntax; and
...
One of the simplest ways to realize an abstract notion of homotopy theory is through categories with weak equivalences. A \textbf{category with weak equivalences} is a category $C$ equipped with a wide subcategory $\cW$, whose maps we call weak equivalences. We can think of the weak equivalences as the maps which are invertible up to homotopy (whatever that may mean in this context) but not necessarily invertible in $C$. We can then construct the homotopy category $\Ho C$ by freely inverting all the maps in $\cW$. While this category correctly realizes the homotopical structure that we are interested in, it is in general difficult to work with, so we work with it indirectly by considering the pair $(C,\cW)$. We call a functor between categories with weak equivalences which preserves weak equivalences \textbf{homotopical}, and we denote the category of categories with weak equivalences and homotopical functors as $\weCat$.

\begin{rem}
    The definition for categories with weak equivalences given above is sometimes used for relative categories, while categories with weak equivalences often require $\cW$ to contain all isomorphisms and be closed under 2-out-of-3. For this paper, we just use the term categories with weak equivalences to match \cite{KL18}, where the distinction between these two terms is not particularly relevant.
\end{rem}

The category $\weCat$ can itself be though of as a category with weak equivalences, where the weak equivalences are given by Dwyer-Kan equivalences (DK-equivalences). A DK-equivalence is a homotopical functor $F\colon C\to D$, which when we take the simplicial localization $L\colon\weCat\to\sCat$ (which can be implemented in a number of ways e.g. see \cite{BK}), it descends to an equivalence of homotopy categories and is $\infty$-full and faithful, i.e. for all $x,y\in LC$ the simplicial map
$$ LF_{x,y}\colon LC(x,y)\to LD(LFx,LFy) $$
is a weak homotopy equivalence (of simplicial sets). This definition is supposed to, in a sense, tell us that $C$ and $D$ encode the same homotopy theory i.e. they have the same objects (up to homotopy equivalence) and the spaces of maps between objects are equivalent.
...
We denote the full subcategory of $\sSet$ spanned by quasicategories by $\qCat$. We additionally write $\tau_1\colon\sSet\to\Cat$ for the adjoint to the nerve functor, and we let $\qCat_2$ denote the 2-category of quasicategories, where the hom-categories are given by $\qCat_2(C,D):=\tau_1(D^C)$.

To study the homotopy theory of quasicategories, we need a notion of weak equivalences which will be given by categorical equivalences of quasicategories. To define these, we first establish the notion of an $E[1]$-homotopy, where $E[1]$ denotes the simplicial set given by the nerve of the contractible groupoid on two objects.
\begin{defn}
    Two maps $f,g\colon X\to Y$ of simplicial sets are $E[1]$-\textbf{homotopic}, denoted $f\sim_{E[1]} g$, if there exists a simplicial map $H\colon X\times E[1]\to Y$ whose restriction to $X\times\partial\Delta_1$ is $[f,g]$. A map $f\colon X\to Y$ is then a \textbf{categorical equivalence} if there exists a map $g\colon Y\to X$ such that $fg\sim_{E[1]}1_Y$ and $gf\sim_{E[1]}1_X$.
\end{defn}
With this we now have some notion of a homotopy theory on the semantic side of our argument. However, we are not just interested in infinity categories in general. At the core of the relationship between type theories and categories is the notion that the presence of type constructors on a theory should correspond to imposing more categorical structure on the semantic side. In the case that we are interested in, the categorical structure that we expect is that of a locally Cartesian closed infinity category, a notion analogous to that of a locally Cartesian closed category, but requires some setup for our particular choice of model.

We define the \textbf{join} of simplicial sets as the unique functor $\star\colon\sSet\times\sSet\to\sSet$ with natural transformations \(K\to K\star L\leftarrow L\) such that $\Delta_m\star\Delta_n\cong\Delta_{m+n+1}$, naturally in both $m$ and $n$, and both functors $-\star L\colon\sSet\to L\downarrow\sSet$ and $K\star-\colon\sSet\to K\downarrow\sSet$ preserves colimits. By the adjoint functor theorem, $-\star L$ admits a right adjoint which takes a map of simplicial sets $X\to K$ to the simplicial set $K\downarrow X$ defined by:
\[(K\downarrow X)_n=\left\{Y\colon\Delta_n\star L\to K\mathrel{\Big|} Y\vert_L=X\right\}.\]
Instantiating this definition with a 0-simplex $x\colon\Delta_0\to C$, we obtain a notion of a \textbf{slice quasicategory}
\[(C\downarrow x)_n=\left\{X\colon\Delta_{n+1}\to C\mathrel{\Big|}X\vert_{\Delta^{\{n+1\}}}=x\right\}.\]
Note that since the canonical map $C\downarrow x\to C$ lifts against inner horn inclusions, we get that $C\downarrow x$ is a quasicategory.
...
\begin{defn}
    ...
    \begin{enumerate}
        \item A \textbf{cone} over $X$ is a simplicial map $Y\colon K^\lhd\to C$ such that $Y\vert_K=X$.
...
With this, we have fully defined the semantic side of our proposed syntax-semantics relationship, but we still need a bit more homotopy theory in order to study the homotopical aspects of our type theories. To do this, we introduce a slightly weakened version of a model structure:
\begin{defn}\cite[Def. 6.6]{KL18}
    ...
    \begin{enumerate}
        ...
        \item cofibrations have the left lifting property with respect to trivial fibrations, and trivial cofibrations with cofibrant source (i.e. if $A$ is the source, then $0\to A\in\cC$) have the left lifting property with respect to fibrations; and
    ...
    \end{enumerate}
\end{defn}
One should compare this definition to that of a model category, where we have two weak factorization systems $(\cC,\cF\cap\cW)$ and $(\cC\cap\cW,\cF)$. In a left semi-model structure, we still have the first factorization system, but the later is weakened to only consider maps with cofibrant source. As it turns out, much of the machinery of model categories can be leveraged even if we are in this slightly weaker setting, and the existence of a full model structure on contextual categories is unknown, so this is the structure that we will use.
...
\end{lem}
Again, we can compare these hypotheses to that of a model category and a left semi-model category and see that we now have two weak factorization systems, which makes classifying maps a little easier, but the anodyne maps (i.e. the maps in $\cA$) still don't quite match up with the trivial cofibrations.

This finally sets us up with all the theory we need to study the homotopical structure of our type theories, but before we establish the left semi-model structure on contextual categories, we first need to set up exactly how we will be studying the syntax side of the relationship.

\subsection{Semantics of Type Theory}
As we explained before, we don't want to work directly with the syntax for a number of reasons, so the models of type theories that we will be working with is that of contextual categories, as introduced by Cartmell in his thesis \cite{Cartmell}.
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item for each for each $\Gamma\in\Ob_{n+1}C$ and $f\colon\Delta\to\ft\Gamma$, we have $\ft(f^*\Gamma)=\Delta$, and the following square
        \[\begin{tikzcd}
        ...
        \end{tikzcd}\]
        is a pullback; and
        \item these canonical pullbacks are strictly functorial, i.e. for $\Gamma\in\Ob_{n+1}C,\id_{\ft\Gamma}^*\Gamma=\Gamma$ and $\id_{\ft\Gamma}.\Gamma=\id_\Gamma$; and for $\Gamma\in\Ob_{n+1}C,f\colon\Delta\to\ft\Gamma$ and $g\colon\Theta\to\Delta$, we have $(fg)^*\Gamma=g^*f^*\Gamma$ and $fg.\Gamma=f.\Gamma\circ g.f^*\Gamma$.
    \end{enumerate}
\end{defn}
...
\end{defn}
Essentially algebraic theories differ from algebraic theories in that they allow operations to be partially defined. This is necessary, for example, when writing a theory of categories, as the composition operation $\circ\colon\Mor\times\Mor\to \Mor$ is only defined when the source and target of our input morphisms match.

\begin{defn}
    A (set-theoretic) \textbf{model} of an essentially algebraic theory is then an assignment of a set $M(s)$ to every sort $s$, and a partial function $M(\sigma)\colon\Pi_{i\in I}M(s_i)\to M(s)$ to every operation symbol $\sigma\colon\Pi_{i\in I}s_i\to s$, such that:
    \begin{enumerate}
...
We wish to additionally equip our contextual categories with structures corresponding to the various forms of type constructors that appear in MLTT. These include:
\begin{enumerate}
    \item Identity types (denoted $\Id$);
    \item unit types (1);
    \item dependent sum or Sigma types ($\Sigma$); and
...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item for each $\Gamma.A$, a section $\refl_A\colon\Gamma.A\to\Gamma.A.p_A^*A.\Id_A$; and
    ...
    \end{enumerate}
\end{defn}
The first three rules of this type structure are meant to mimic the identity type constructor rules in our type theory, and the last rule ensures that this structure respects substitution. Similar definitions for unit, Sigma, and (extensional) Pi-type structures can be found in \cite[Appendix B]{KL21}.
...
\begin{rem}
    All of the homotopy theoretic constructions that we will be using will only require $\Id$-type structures on our contextual categories, so if we wished, we could work within $\CxlCat_{\Id}$ and port our work up to $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ using the natural free-forgetful adjunction that comes from extending the essentially algebraic theories. However, as we are only interested in the category $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ in this paper, and working in this extension does not meaningfully complicate anything, we will work in slightly less generality.
\end{rem}

Following \cite[Prop. 3.3.1]{Garner}, we can extend the notion of $\Id$-type structures to \textbf{identity contexts}. Specifically, given $\Gamma\in C$ and a context extension $\Gamma.\Delta\in C$, we can further construct a context extension $\Gamma.\Delta.p_\Delta^*\Delta.\Id_\Delta$ which comes equipped with reflexivity map and an elimination term which directly generalize those of the $\Id$-type structure on a single type. This is essentially a notational issue which helps us define a notion of homotopies within our contextual categories.
\begin{defn}
...
\begin{defn}
    ...
    An \textbf{equivalence} $\Gamma\xrightarrow{\sim}\Delta$ in $C$ is a map $f\colon\Gamma\to\Delta$ for which there exists some $g_1,g_2,\eta,\varepsilon$ making it a structured equivalence.
\end{defn}

With this definition of equivalence we can see that contextual categories have an underlying category with weak equivalences (which is in fact even a fibration category \cite[Thm. 3.2.5]{AKL}). We additionally use these equivalences to define a weak equivalence of contextual categories (note that we have moved up a level from looking at a single contextual category as a category with weak equivalences to looking at $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ as a category with weak equivalences).
\begin{defn}
    ...
    \end{enumerate}
\end{defn}
The type-theoretic equivalences will make up the class $\cW$ of our left semi-model structure.

The rest of our classes of maps, and their associated weak factorization systems, which will make up our left semi-model structure, will be cofibrantly generated using the small object argument \cite{Quillen}. To describe the generating cofibrations/anodyne maps, we first need to construct certain freely generated contextual categories. This is possible by our description of contextual categories as models of an essentially algebraic theory.
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item $\gen{\Gamma_n\vdash a:A}$ is freely generated by a context $\Gamma$, a type $A$, and a term $a:A$ in context $\Gamma$, i.e. a section of $p_A$;
        \item $\gen{\Gamma_n\vdash A\simeq A'}$ is freely generated by a context $\Gamma$, types $A,A'$ in context $\Gamma$, and maps $f,g_l,g_r,\alpha_l,\alpha_r$ constituting an equivalence from $A$ to $A'$ over $\Gamma$; and
        \item $\gen{\Gamma_n\vdash e:\Id_A(a,a')}$ is freely generated by $\Gamma$ and $A$, as in $\gen{\Gamma_n\vdash A}$, and a section to the projection map $\Gamma.A.(p_A)^*A.\Id_A\to\Gamma.A$, giving the three terms $a,a'$, and $e$.
    \end{enumerate}
\end{defn}
\begin{rem}
    Our use of the phase "freely generated" is perhaps a bit misleading, as these are not all freely generated objects of the theory of contextual categories. Since terms of the theory correspond to sections, these objects must involve some relations. Calling these objects "freely generated" emphasizes their role in relation to their corresponding syntactic objects.
\end{rem}

We can now define the classes of maps that will make up the rest of the left semi-model structure on $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$.
\begin{defn}
...
\end{thm}
\begin{sketch}
    We show that $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$, along with the supplied classes of maps, satisfy the hypotheses of \ref{lem:semi-model struct}. As noted before, we know that $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ is bicomplete, as a category of models of an essentially algebraic theory, and as we saw above, we have two weak factorization systems, $(\cC,\cT\cF)$ and $(\cA,\cF)$, constructed with the small objects argument. Showing that the class $\cW$ satisfies the correct closure properties and that $\cT\cF=\cF\cap\cW$ are straightforward arguments, so the bulk of the work that is required is in showing that maps with cofibrant source are anodyne exactly when they are both cofibrations and weak equivalences.
    ...
    We can factor the reflexivity homotopy $c_h\colon A\to Y^{\eqv}$ for $h$ through a pullback with the following diagram:
    \[\begin{tikzcd}
            & {(h,1_Y)^*Y^{\eqv}} \arrow[r] \arrow[d, two heads, "{(Q_0,Q_1)}"] \arrow[dr, phantom, "\lrcorner", pos=0] & Y^{\eqv} \arrow[d, two heads, "{(P_0,P_1)}"]\\
        A \arrow[ur, "c_h'"] \arrow[r, "{(1_A,h)}"] \arrow[dr, "1_A"'] \arrow[urr, bend left=49, "c_h"] & A\times Y \arrow[r, "{(h,1_Y)}"] \arrow[d, two heads, "\pi_0"] \arrow[dr, phantom, "\lrcorner", pos=0.1] & Y\times Y \arrow[d, two heads, "\pi_0"]\\
            & A \arrow[r, "h"] & Y
    \end{tikzcd}.\]
...
With our syntax given by contextual categories and semantics given by quasicategories, we finally have the background to start discussing the conjecture which is the primary motivation of this research. We saw previously that each contextual category has an underlying category with weak equivalences, giving us an inclusion $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}\to\weCat$. It is known that the categories $\weCat$ and $\qCat$ are DK-equivalent as categories with weak equivalences. We will call this equivalence $\Ho_\infty$ and note that it can be implemented in a number of ways (see \cite[$\S1.6$]{Barwick}), but, for concreteness, we can consider the following localization construction which takes a category with weak equivalences $(C,\cW)$, takes the pushout of the span
$$ \coprod_{w\in\cW}E[1]\leftarrow\coprod_{w\in\cW}\Delta^1\to NC $$
in $\sSet$ ($NC$ is the nerve of $C$), and then takes a fibrant replacement in the Joyal model structure. We can then define the map $\Cl_\infty$ as the following composite:
...
To get the desired functor from $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ to $\qCat$ then, all that is necessary is to find the correct simplicial object in $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ which induces a nerve functor that is valued in quasicategories and equivalent to $\Cl_\infty$. There is an "obvious" cosimplicial object $\T$ in $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$ whose theories are freely generated, for each $n\in\N$, by closed types $A_i$, for each $i\in[n]$, and terms $x:A_i\vdash f_{i+1}(x):A_{i+1}$, for each $i\in[n-1]$,. The face maps $\delta_i\colon\T_{n-1}\to\T_n$ are given by sending $f_i$ to $f_{i+1}\circ f_i$, and the degeneracy maps $\sigma_i\colon\T_{n+1}\to\T_n$, are defined by sending $f_{i+1}$ to $\id_{A_i}$.
...
This situation is analogous to that of the homotopy coherent nerve for simplicial categories (see \cite[Ch. 1]{Lurie} for more exposition), where the naive inclusion $\Delta\hookrightarrow\Cat\hookrightarrow\sCat$, does not produce a nerve functor that properly accounts for the homotopical data, i.e. the simplicial enrichment, on $\sCat$. The solution to this is then to consider "thickened" simplices which account for the homotopical structure and are a Reedy cofibrant replacement of the naive cosimplicial object. We wish to attempt something similar in our context, but to make sure that a Reedy cofibrant replacement exists, we need some sort of Reedy left semi-model structure on $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}^\Delta$.
...
\end{defn}
\begin{defn}
    Let $\cR$ be a Reedy category, $\cM$ a model category (or more generally a category with a class of (co)fibrations), and $f\colon X\to Y$ a map of diagrams in $\cM^\cR$.
    \begin{enumerate}
...
While the standard Reedy model structure for diagrams valued in a model category is well understood, we only have a left semi-model structure on $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$, so we wish to construct an analogous Reedy left semi-model structure on $\cM^\cR$ when $\cM$ does not have a full model structure.

\begin{thm}\label{thm:reedy left-semi model struct}
    Let $\cM$ be a category satisfying the hypotheses of \ref{lem:semi-model struct}, and $\cR$ a Reedy category. The category $\cM^\cR$ comes equipped with a left semi-model structure where weak equivalences are defined component-wise, and the (co)fibrations are the Reedy (co)fibrations.
\end{thm}
...
\begin{sketch}
    (Theorem \ref{thm:reedy left-semi model struct}) The proof mostly follows from standard inductive arguments using the Reedy structure of $\cR$ (compare to similar proofs in \cite[Ch. 15]{Hirshhorn}, \cite[Ch. 9]{RadulescuBanu}).
