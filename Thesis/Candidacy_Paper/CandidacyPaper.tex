...
We denote the full subcatefory of $\sSet$ spanned by quasicategories by $\qCat$. We write $\tau_1\colon\sSet\to\Cat$ for the adjoint to the nerve functor, and we let $\qCat_2$ denote the 2-category of quasicategories, where the hom-categories are given by $\qCat_2(C,D):=\tau_1(D^C)$.

At the core of the realtionship between type theories and categories is the notion that the type formers on theory should correspond to imposing more categorical structure on the semantic side. In the case that we are interested in, the categorical structure that we expect is that of a locally Cartesian closed infinty category, a notion analogous to that of a locally Cartesian closed category, but requires some setup for our particular choice of model.
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        %     \node (A) at (0,0) {$f^*\Gamma$};
        %     \node (X) at (2,0) {$\Gamma$};
        %     \node (B) at (0,-2) {$\Delta$};
        %     \node (Y) at (2,-2) {$\ft\Gamma$};
        %     \draw[->] (A) to node[left]{$p_{f^*\Gamma}$} (B);
        %     \draw[->] (A) to node[above]{$f.\Gamma$} (X);
        %     \draw[->] (X) to node[right]{$p_\Gamma$} (Y);
        %     \draw[->] (B) to node[above]{$f$} (Y);
        % \end{tikzpicture}\end{matrix};\] % How do I get the pb arrow?
        \[\begin{tikzcd}
