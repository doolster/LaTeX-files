...
A presentation for the structural rules of Martin-L\"of Type Theory (MLTT), which is what we will consider to be the most "basic" dependent type theory, along with the rules for other common type constructors can be found in \cite[Appendix A]{KL21}.
...
As briefly referenced in the previously noted result, when studying type theories, we often do not study the syntax directly, but rather, we consider semantics models which closely model the structures that we find present in the syntactic categories of type theories.


Rather than studying the syntax of dependent type theory in isolation, this paper emphasizes semantic models such as contextual categories. A contextual category is a categorical abstraction of the syntactic category of a type theory: it encodes contexts, types in context, terms as sections, and substitution in a purely algebraic way. Studying these semantic models has several advantages:
- it gives an invariant notion of model independent of the particular presentation of syntax;
- it produces a category of models with natural morphisms between them, which is essential for comparing theories and constructing homotopies;
- it makes available categorical tools such as limits, adjoints, and model structures that are hard to see directly in raw syntax;
- it isolates the structural content of the theory from the bureaucratic details of derivations and proof-term representations.

%In particular, contextual categories can be described as models of an essentially algebraic theory, so the collection of all such models forms a well-behaved category.  This allows us to study type theories by studying their semantics directly, rather than by manipulating syntactic derivations.  When we pass to homotopical questions, this semantic perspective becomes even more important, because it lets us equip the category of models with a notion of weak equivalence and to relate type-theoretic equivalence to categorical equivalence.

The remainder of the paper therefore begins by fixing the minimal syntax and notation needed for our arguments, then explains how extensional dependent type theory corresponds to locally cartesian closed structure, and finally develops the semantic framework of contextual categories as the primary objects of study. This sets the stage for the later sections, where we build homotopical models and compare them with the syntactic presentations they are meant to represent.
