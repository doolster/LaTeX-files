...
Type theory is a foundation for mathematics which formalizes, and notably differentiates, the notion of mathematical terms, say the number 2 or a function $f$, and the types which those terms belong to, say the natural numbers or functions $\N\to\N$. Dependent type theory is then a type theory in which the basic judgments are not only that a term has a type, but that a type itself may depend on a surrounding context of assumptions. In the fragment we reference throughout this paper, a typical judgment has the form
\[ \Gamma\vdash A\ \text{type}\qquad\text{or,}\qquad\Gamma\vdash a : A, \]
where $\Gamma$ is a context, i.e. a list of typed variables $\Gamma=(x_1:X_1,\dots,x_n:X_n)$, and $a$ or $A$ may implicitely depend on the variables $x_i$. The basic structural rules of type theory can be extended by the inclusion of type constructors which allow us access to various constructions within our theory such as a unit type, denoted 1, dependent pair and function types, called $\Sigma$ and $\Pi$ types respectively, or the type of identities on a given type. These constructions are asserted by adding rules such as the following for $\Id$-types to our theory.
...
A central value of dependent type theory is that it naturally admits a syntax-semantics relationship with category theory. On the syntactic side, contexts and substitutions naturally organize into a category we call the syntactic category of a type theory whose objects are contexts and whose morphisms from a context $\Gamma$ to a context $\Delta$, are lists of terms of the types in $\Delta$ derived in the context $\Gamma$. On the other side, we can get a type theory by considering the internal logic of a category with sufficient structure.
...
\begin{rem}
    The definition for categories with weak equivalences given above is sometimes used for relative categories, while categories with weak equivalences require $\cW$ to contain all isomorphisms and be closed under 2-out-of-3. For this paper, we just use the term categories with weak equivalences to match \cite{KL18}, where the distinction between these two terms is not particularly relevant.
\end{rem}

The category $\weCat$ can itself be though of as a category with weak equivalences, where the weak equivalences are given by Dwyer-Kan equivalences (DK-equivalences). A DK-equivalence is a homotopical functor $F\colon C\to D$, which when we take the simplicial localization $L\colon\weCat\to\sCat$ (which can be implemented in a number of ways e.g. see \cite{BK}), it descends to an equaivalence of homotopy categories and is $\infty$-full and faithful, i.e. for all $x,y\in LC$ the simplicial map
$$ LF_{x,y}\colon LC(x,y)\to LD(LFx,LFy) $$
is a weak homotopy equivalence (of simplicial sets). This definition is supposed to, in a sence, tell us that $C$ and $D$ encode the same homotopy theory i.e. they have the same objects (up to homtopy equivalence) and the spaces of maps between objects are equivalent.
...
To study the homotopy theory of quasicategories, we need a notion of weak equivalences which will be given by categorical equivalences of quasicategories. To define these, we first establish the notion of an $E[1]$-homotopy, where $E[1]$ is the nerve of the contractible groupoid on two objects.
\begin{defn}
    Two maps $f,g\colon X\to Y$ of simplicial sets are $E[1]$-\textbf{homotopic}, denoted $f\sim g$, if there exists a simplicial map $H\colon X\times E[1]\to Y$ whose restriction to $X\times\partial\Delta_1$ is $[f,g]$. A map $f\colon X\to Y$ is then a \textbf{categorical equivalence} if there exists a map $g\colon Y\to X$ such that $fg\sim1_Y$ and $gf\sim1_X$.
\end{defn}
...
With this, we have fully defined the semantic side of our proposed syntax-semantics relationship, but we still need a bit more homotopy theory in order to study the homotopical aspects of our type theories. To do this, we introduce a slightly weakened version of a model category:
...
This finally sets us up with all the theory we need to study the homotopical structure of our type theories, but before we establish the left semi-model structure on contextual categories, we first need to set up exactly how we will be studying the syntax side of our realtionship.

\subsection{Semantics of Type Theory}
As we explained before, we don't want to actually work directly with the syntax for a number of reasons, so the models of type theories that we will be working with is that of contextual categories, as introduced by Cartmell in his thesis \cite{Cartmell}.
\begin{defn}
...
\note{move or add transition to above section?}
...
While the standard Reedy model structure for diagrams valued in a model category is well understood, since we only have a left semi-model structure on $\CxlCat_{\Id,1,\Sigma,\Pi_{\ext}}$, we wish to construct an analogous Reedy left semi-model structure on $\cM^\cR$ when $\cM$ does not have a full model structure.
